...
class QueueService(IQueueService):
    ...
    def join_restaurant_waiting_queue(self, restaurant_id: int, user_id: int) -> JoinQueueResponse:
        ...
        # 計算預估時間
        avg_dining_time, total_seats= self.queue_runtime_repo.get_metrics(restaurant_id=restaurant_id)
    ...
    def leave_restaurant_waiting_queue(self, restaurant_id: int, user_id: int) -> None:
        # user 是否已在任何餐廳排隊
        current_queue_restaurant_id = self.queue_repo.get_user_current_queue(user_id=user_id)
        if current_queue_restaurant_id is None:
            raise NotInQueueError()
        # 餐廳是否存在
        exist_restaurant = self.map_repo.check_exists(restaurant_id=restaurant_id)   
        if not exist_restaurant :
            raise RestaurantNotFoundError()
        if current_queue_restaurant_id != restaurant_id:
